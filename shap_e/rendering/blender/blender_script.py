...
def pc_to_mesh(filepath):

    # code to render the mesh of the shapetalk dataset objects
    # the code was taken from https://github.com/optas/changeit3d/blob/main/changeit3d/external_tools/blender_based_visualization/blender_pointcloud.py 
    ...
    pts = swap_axes_of_pointcloud(np.load(filepath)['pointcloud'], [0, 2, 1])
    pt_colors = (np.array([128, 128, 128]) / 255) * np.ones_like(pts)

    sphere_mesh = bpy.data.meshes.new('sphere')
    sphere_bmesh = bmesh.new()
    bmesh.ops.create_icosphere(sphere_bmesh, subdivisions=2, radius=0.015)
...
def create_vertex_color_shaders_gray():
    # By default, Blender will ignore vertex colors in both the
    # Eevee and Cycles backends, since these colors aren't
    # associated with a material.
    #
    # What we do here is create a simple material shader and link
    # the vertex color to the material color.
    print(f"ENTERED CREATING VERTEX COLOR SHADERS")
    for obj in bpy.context.scene.objects.values():
        if not isinstance(obj.data, (bpy.types.Mesh)):
            continue

        if len(obj.data.materials):
            print(f"OBJECT {obj} HAS MATERIALS")
        ...
        if not len(color_keys):
            ...
            print(f"CREATED NEW COLOR KEY {obj.data.vertex_colors.keys()}")
            ...
            for poly in obj.data.polygons:
                for idx in poly.loop_indices:
                    ...
                    i += 1
            print(f"SET COLORS TO GRAY {obj.data.vertex_colors.values()}")
            color_keys = (obj.data.vertex_colors or {}).keys()
            # Many objects will have no materials *or* vertex colors.
            #continue
        ...
        print(f"socket_map[Base Color] - {socket_map['Base Color']}")
        mat.node_tree.links.new(v_color.outputs[0], socket_map["Base Color"])
...
def save_rendering_dataset(
    ...
    create_camera()
    if make_gray:
        create_vertex_color_shaders_gray()
    else:
...
def main():
    ...
    print(f"make gray: {args.make_gray}")
